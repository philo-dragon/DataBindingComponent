    ...
    private CharSequence getNewTextByConfig() {
        ...
        switch (mCurrState) {
            ...
            case STATE_EXPAND: {
                ...
                //获取Drawable资源
                Drawable d = getResources().getDrawable(R.mipmap.less);
                d.setBounds(0, 0, d.getIntrinsicWidth(), d.getIntrinsicHeight());
                //创建ImageSpan
                ImageSpan span = new ImageSpan(d, ImageSpan.ALIGN_BASELINE);
                //用ImageSpan替换文本
                ssbExpand.setSpan(span, ssbExpand.length() - getLengthOfString(mToShrinkHint), ssbExpand.length(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
